...
export function dalVerifySuccess<T, E extends DalError>(
	...
	const res = dalThrowError(
		dalUnauthorizedRedirect(dalLoginRedirect(dalReturn), unauthorizedRedirectPath),
	);
	return res.data;
}
...
export function dalFormatErrorMessage(error: DalError) {
	...
	switch (error.type) {
		...
		case "drizzle-error":
			return `A database error occurred ${error.error.message}`;
		case "better-auth-error":
			return `A auth error occured: ${error.error.message}`;
		case "unknown-error":
